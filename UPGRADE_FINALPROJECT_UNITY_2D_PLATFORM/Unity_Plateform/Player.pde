//YodaLight64 helped in line 58, 150-160.
...
class Player {
  ...
  void inputCheck() {
    ...
    if(theKeyboard.holdingLeft || potValue <=127) {
      velocity.x -= speedHere;
    } else if(theKeyboard.holdingRight || potValue >=128) {
      velocity.x += speedHere;
    } 
    ...
    if(isOnGround) { // player can only jump if currently on the ground
      if(theKeyboard.holdingSpace || theKeyboard.holdingUp || buttonValue == 1) { // either up arrow or space bar cause the player to jump
